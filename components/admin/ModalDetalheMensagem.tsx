...
export default function ModalDetalheMensagem({
  isOpen,
  onClose,
  item,
}: ModalDetalheMensagemProps) {
  if (!isOpen) return null;

  const servico =
    item.itemType === "lead"
      ? item.tipoDecl || "Consultoria IRPF"
      : item.assunto || "Consultoria IRPF";
  const origem = item.origem || "site";
  const telefone = item.telefone || "";
  const whatsappLink = telefone
    ? buildWhatsAppLink(telefone, item.nome, servico, origem)
    : "";
  const suggestedMessage = formatWhatsAppMessage(
    telefone,
    item.nome,
    servico,
    origem,
            ...
            {item.email && (
              <div>
                ...
                <a
                  ...
                  className="text-sm hover:text-[#C6FF00] transition-colors break-all"
                >
                  {item.email}
                </a>
              </div>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div>
                          ...
                          <p className="text-[10px] uppercase tracking-widest opacity-40 mt-1">
                            {relatedItem.itemType} • {relatedItem.origem} •{" "}
                            {new Date(relatedItem.createdAt).toLocaleString(
                              "pt-BR",
                              {
                                day: "2-digit",
                                month: "2-digit",
