...
const servicos = [
  ...
  },
  {
    ...
    desc: "Declaração anual (DASN-SIMEI), abertura, cancelamento, parcelamento de DAS e IRPF do MEI. Atendemos microempreendedores em todo o Brasil.",
    tag: "MEI",
    href: "/mei",
    external: false,
  },
    ...
    <section id="servicos" className="max-w-7xl mx-auto px-6 py-14">
      ...
      <div className="space-y-0">
        {servicos.map((s, i) => {
          ...
          const className =
            "group flex flex-col md:flex-row md:items-center gap-4 md:gap-8 py-6 editorial-border cursor-pointer";
          return s.external ? (
            <motion.a
              key={s.num}
              href={s.href}
            ...
            <motion.div
              ...
              transition={{ duration: 0.4, delay: i * 0.1 }}
            >
              <Link href={s.href} className="contents">
                {inner}
              </Link>
            </motion.div>
          );
